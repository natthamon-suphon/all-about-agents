# Antigravity Repository Entry Point
...
This surface has two facts the others do not. Antigravity documents no
`SessionStart` event, so the routing contract is inlined into the rendered
`GEMINI.md` rather than injected by a hook. The deploy of that file is
guarded, so a completed registration can leave the live file untouched. Ask the
product whether a `Routing contract` section is loaded; do not infer it.
