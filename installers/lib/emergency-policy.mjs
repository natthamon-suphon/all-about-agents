...
function verifiedDisposableTargets(rootValue, pathEntries) {
  ...
  if (!resolved || !attestation || !isAbsolute(root) || pathEntries.length === 0) return false;
...
function recursiveErase(command, capability, pathEntries) {
  const commandText = normalized(command);
  const capabilityText = normalized(capability);
  const recursive = /(?:^|\s)(?:rm|rmdir|del|erase|shred|remove-item)(?:\s|$)/u.test(commandText)
    && /(?:^|\s)(?:-r|-R|-rf|-fr|-recurse|--recursive|--recurse|\/s)(?:\s|$)/u.test(commandText)
    || /(?:recursive|erase|wipe|delete[-_ ]all|filesystem[-_ ]root)/u.test(capabilityText);
...
function hasRawDiskDestruction(command, capability, operation) {
  ...
  return /(?:\b(?:format|fdisk|diskpart|mkfs|dd)\b|(?:raw|physical)[-_ ]?(?:disk|device)|(?:partition|volume)[-_ ]?(?:delete|destroy|format)|\\\\\.\\[a-z]:)/u.test(all)
...
function hasForcePush(command, operation) {
  ...
  return /\bgit\s+push\b[^\n]*(?:--force(?:-with-lease)?|\s-f(?:\s|$))/u.test(all)
    || /\bforce[-_ ]?push\b/u.test(all);
}

function hasHistoryRewrite(command, operation) {
  const all = `${normalized(command)} ${operationText(operation)}`;
  return /\bgit\s+(?:rebase|filter-branch|filter-repo|replace|reflog\s+expire)\b/u.test(all)
    || /\bgit\s+commit\b[^\n]*\s--amend(?:\s|$)/u.test(all)
    || /\b(?:history[-_ ]rewrite|rewrite[-_ ]history|amend[-_ ]commit)\b/u.test(all);
}

function hasDiscard(command, operation) {
  const all = `${normalized(command)} ${operationText(operation)}`;
  return /\bgit\s+(?:clean\s+-[a-z]*f|checkout\s+--|restore\s+--|reset\s+--hard|stash\s+(?:drop|clear))\b/u.test(all)
...
function hasCredentialAccess(command, capability, operation, pathEntries) {
  const all = `${normalized(command)} ${normalized(capability)} ${operationText(operation)}`;
  const pathHit = pathEntries.some((entry) => SECRET_PATH.test(cleanPath(entry.path)) && !EXAMPLE_PATH.test(cleanPath(entry.path)));
  return pathHit || /(?:credential|secret|token|private[_-]?key|secret[-_ ]store|password[-_ ]store)/u.test(all)
...
function hasSecretOutput(command, capability, operation, pathEntries) {
  const all = `${normalized(command)} ${normalized(capability)} ${operationText(operation)}`;
  const secretPath = pathEntries.some((entry) => SECRET_PATH.test(cleanPath(entry.path)) && !EXAMPLE_PATH.test(cleanPath(entry.path)));
...
function hasGuardrailBypass(command, capability, operation) {
  const all = `${normalized(command)} ${normalized(capability)} ${operationText(operation)}`;
  return /(?:dangerously-skip-permissions|bypass[-_ ]?permissions|disable[-_ ]?(?:safety|guard|hook|deny)|skip[-_ ]?(?:safety|guard|hook|verification)|hooks?\s+(?:off|disable)|permissions?\s+(?:off|disable)|--no-verify\b)/u.test(all);
}

function ruleIds(policy) {
  if (!isPlainObject(policy)) return DEFAULT_RULE_IDS;
  const ids = Array.isArray(policy.orderedRuleIds) ? policy.orderedRuleIds.filter((id) => typeof id === "string") : [];
  return ids.length > 0 ? ids : DEFAULT_RULE_IDS;
}
...
export function classifyEmergencyAction(input = {}) {
  ...
  const pathEntries = valuesFromPaths(paths);
  const checks = {
    "filesystem-root-erasure": () => {
      if (!recursiveErase(command, capability, pathEntries)) return false;
