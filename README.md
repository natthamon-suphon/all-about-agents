...
Use `--surface antigravity|claude|codex|all` and an explicit
`--destination-root` for qualification. Each surface also has a documented
environment-based root, but the setup pages use disposable roots so a real user
configuration is not changed. Claude reads `CLAUDE_CONFIG_DIR` and Codex reads
`CODEX_HOME`. Antigravity documents no variable of its own, so this repository
...
and `<root>/codex`. This keeps surface files from colliding. Adding the third
surface changed what `--surface all` means: an existing machine that reruns it
now renders one more package.
...
- [Antigravity manifest](installers/manifests/antigravity.json)
