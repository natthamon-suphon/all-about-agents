...
This skill owns the artifact: workflow specs in `workflows/*.md`. Do not divert into `all-about-agents:brainstorming` — that skill produces a design doc and hands off to implementation, which is not what this is for.
...
- `NOTES.md` — raw notes on your human partner's world: the tools they use, the channels they process, and their own terminology for both. When it is empty or thin, interview them about their world before specifying anything. Sharpen fuzzy terms into canonical ones as they surface, and record them here.
