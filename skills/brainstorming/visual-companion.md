...
#           "screen_dir":"/path/to/project/.claude/all-about-agents/brainstorm/12345-1706000000/content",
#           "state_dir":"/path/to/project/.claude/all-about-agents/brainstorm/12345-1706000000/state"}
```
...
**Finding connection info:** The server writes its startup JSON to `$STATE_DIR/server-info`. If you launched the server in the background and didn't capture stdout, read that file to get the URL and port. When using `--project-dir`, check `<project>/.claude/all-about-agents/brainstorm/` for the session directory.

**Note:** Pass the project root as `--project-dir` so mockups persist in `.claude/all-about-agents/brainstorm/` and survive server restarts. Without it, files go to `/tmp` and get cleaned up. Remind the user to add `.claude/` to `.gitignore` if it's not already there.
...
If the session used `--project-dir`, mockup files persist in `.claude/all-about-agents/brainstorm/` for later reference. Only `/tmp` sessions get deleted on stop.
