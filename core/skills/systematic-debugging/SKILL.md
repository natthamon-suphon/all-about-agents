...
1. Classify the request before changing anything. A failure, regression,
   ...
   does not; use the requested implementation workflow instead.
...
4. **Gather Evidence in Multi-Component Systems**
   ...
   **Safe multi-layer evidence example:**
   ```bash
   # Layer 1: Workflow
   printf '%s\\n' "workflow boundary: input metadata present"

   # Layer 2: Build script
   printf '%s\\n' "build boundary: artifact path=<REDACTED>"

   # Layer 3: Signing script
   printf '%s\\n' "signing boundary: exit code=<REDACTED>"
...
Do not turn an external or nondeterministic limitation into a confident
diagnosis. Record what was checked, the smallest available evidence, and what
remains `not run`.
