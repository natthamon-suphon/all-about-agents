...
test("Codex preserves supplied skill content and marks the next missing canonical skill deferred", () => {
  ...
  const deferred = files.get(".agents/skills/codebase-design/SKILL.md");
...
test("Codex reports absent and whitespace canonical skill sources separately", () => {
  ...
  assert.ok(absent.diagnostics.some((diagnostic) => /codebase-design/u.test(diagnostic.message) && /no source record/u.test(diagnostic.message)));
  const whitespaceCore = {
    ...core,
    skills: [...core.skills, { id: "codebase-design", description: "Whitespace source", content: " \r\n\t " }]
  };
  const whitespace = resultFor("portable", { core: whitespaceCore });
  const diagnostics = whitespace.diagnostics.filter((diagnostic) => /codebase-design/u.test(diagnostic.message));
  assert.equal(diagnostics.length, 1);
  assert.equal(diagnostics[0].code, "missing-skill-source");
  assert.match(diagnostics[0].message, /no usable source content/u);
  assert.match(fileMap(whitespace).get(".agents/skills/codebase-design/SKILL.md"), /DEFERRED: canonical source is missing/u);
});
