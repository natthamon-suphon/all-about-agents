...
test("Codex preserves supplied skill content and marks the next missing canonical skill deferred", () => {
  ...
  const deferred = files.get(".agents/skills/performance-profiling-and-benchmarking/SKILL.md");
...
test("Codex reports absent and whitespace canonical skill sources separately", () => {
  ...
  assert.ok(absent.diagnostics.some((diagnostic) => /performance-profiling-and-benchmarking/u.test(diagnostic.message) && /no source record/u.test(diagnostic.message)));
  const whitespaceCore = {
    ...core,
    skills: [...core.skills, { id: "performance-profiling-and-benchmarking", description: "Whitespace source", content: " \r\n\t " }]
  };
  const whitespace = resultFor("portable", { core: whitespaceCore });
  const diagnostics = whitespace.diagnostics.filter((diagnostic) => /performance-profiling-and-benchmarking/u.test(diagnostic.message));
  assert.equal(diagnostics.length, 1);
  assert.equal(diagnostics[0].code, "missing-skill-source");
  assert.match(diagnostics[0].message, /no usable source content/u);
  assert.match(fileMap(whitespace).get(".agents/skills/performance-profiling-and-benchmarking/SKILL.md"), /DEFERRED: canonical source is missing/u);
});
